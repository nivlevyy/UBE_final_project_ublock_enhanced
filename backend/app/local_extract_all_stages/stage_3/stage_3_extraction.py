...
def detect_suspicious_js_behavior(soup: BeautifulSoup, base_domain: str) -> tuple:
    ...
    for script in inline_scripts:
        try:
            content = (script.get_text() or "").strip().lower()
        except Exception:
            content = ""
    ...
    try:
        ...
        logging.error(f"[ERROR] Problem reading <script src=...>: {e}")
        external_scripts = []

    js_external = len(external_scripts)
    js_total = inline_scripts_count + js_external

    # Ratios: behave = cross-domain external / total JS, risk_patterns = (hi+mid+low) / inline
    js_behave_ratio = (sus_js_domain_not_the_same / js_total) if js_total > 0 else 0.0
    js_risk_patterns_ratio = (
        (high_risk_patterns_count + medium_risk_patterns_count + low_risk_patterns_count) / inline_scripts_count
        if inline_scripts_count > 0 else 0.0)

    return (js_total, inline_scripts_count, js_external, high_risk_patterns_count,
        medium_risk_patterns_count, low_risk_patterns_count, sus_js_domain_not_the_same, js_behave_ratio, js_risk_patterns_ratio)
...
def find_html_features_separated(soup: BeautifulSoup, url: str, feature_type: str, driver: webdriver): # need to add back as arument , driver: webdriver
    ...
    if feature_type == "favicon_check":
        ...
        return ( extern_meta, sus_words_meta, external_meta_ratio,
            extern_script, sus_words_script, external_script_ratio,
            external_link_count, extern_link_ratio,
            total_links, total_extern)
...
#
# def extract_stage3_features_debug_separated(phish_check_list, pid):
#
#     column=["URL"]
#     df = pd.DataFrame(phish_check_list,columns=column)
#     total = len(phish_check_list)
#     results = []
#
...
#     ]
#
#     logging.info(f"\n✅ Finished. stage 3 extraction complete.")
#     phish_stage_3_df =pd.DataFrame(results, columns=headers)
#     return phish_stage_3_df
    ...
    try:
        ...
        for url in url_list:
            for attempt in range(2):
                try:
                    print(f"[PID {os.getpid()}] start {url}")
                    driver.get(url)
                    WebDriverWait(driver, 10).until(
                        lambda d: d.execute_script("return document.readyState") == "complete"
                    )
                    ...
                    results.append(features)
                    print(f"[PID {os.getpid()}] done: {url}")
...
def stage_3_extraction(url_list, proc=3):

    chunks = split_list(url_list, proc)

    with Pool(proc) as pool:
        list_of_results = pool.map(process_multiple_urls, chunks)

    results = []
    for partial_results in list_of_results:
        if partial_results:
    ...
    return df
def proc_ext_3(d:dict,phish_list):
    d["stage_3"]=stage_3_extraction(phish_list)
...
if __name__ == "__main__":
    ...
    my_new_df = stage_3_extraction(url_list)
