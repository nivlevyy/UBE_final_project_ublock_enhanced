...
const updateUBEStatus = async function() {
    ...
    try {
        ...
        else if (status.hasResult) { // should show the prediction in the future
...
if (browser.runtime) {
    browser.runtime.onMessage.addListener((message) => {
        if (message?.source === 'ubeBackgroundScript' &&
            message?.what === 'analysisComplete' &&
            message?.tabId === popupData.tabId) {
            updateUBEStatus();
        }
    });
}
