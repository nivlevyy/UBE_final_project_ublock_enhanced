...
// >>>>> start of local scope
    ...
    const popupDataFromTabId = function (tabId, tabTitle) {
        ...
        const r = {
            ...
            ubeEnabled: µb.UBE ? µb.UBE.enabled : false
        };
    ...
    const onMessage = function (request, sender, callback) {
        ...
        switch (request.what) {
            ...
            case 'openUBEPopupWindow':
                if (µb.UBE && µb.UBE.openPopupWindow) {
                    µb.UBE.openPopupWindow(request.tabId, request.hostname);
                }
                else {
                    ubolog('🚨 UBE Debug: openUBEPopupWindow message received but UBE is not enabled!');
                }
                return;
        ...
        switch (request.what) {
            ...
            case 'toggleUBE':
                ubolog('🚨 UBE Debug: toggleUBE message received, request.state = ', request.state);
            ...
            case 'getUBEStatus':
                if (µb.UBE) {
                    response = {
                        processing: µb.UBE.activeProcessing.has(request.tabId),
                        hasResult: µb.UBE.tabResults.has(request.tabId)
                    };
                } else {
