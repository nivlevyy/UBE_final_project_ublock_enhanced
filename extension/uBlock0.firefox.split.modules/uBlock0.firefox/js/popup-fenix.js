...
const toggleUBE = async function(ev) {
    ...
    //     console.log("soup mobile");
    //     return;
    // }

    const newState = !dom.cl.has(target, 'on');
    dom.cl.toggle(target, 'on', newState);

    updateUBEToggleUI(newState);
...
const openUBEDebugPopup = function(ev) {
    ...
    // this prevents opening results
    // when right clicked if UBE is disabled
    //if (popupData.ubeEnabled) {
    //console.log('UBE Debug: Popup clicked.');

    //const debugUrl = vAPI.getURL(`popupmoogzam.html?tabId=${popupData.tabId}&url=${encodeURIComponent(popupData.pageURL)}`);
...
const updateUBEToggleUI = function(enabled) {
...
const renderUBEStatus = function(status) {
    ...
    else if (status.hasResult) { // should show the prediction alongside it
...
const updateUBEStatus = async function() {
    messaging.send('popupPanel',
        {
            what: 'ubeGetStatus',
            tabId: popupData.tabId}).then((status) => {
...
const initializeUBE = function() {
    ...
    dom.cl.toggle(toggle, 'on', isEnabled);

    updateUBEToggleUI(isEnabled);
};

if (browser.runtime) { // for instant UBE status updating
