...

{
    ...
    const updateBadge = (tabId) => {
        ...
        if ( µb.UBE && µb.UBE.enabled) {
            const isProcessing = µb.UBE.isActiveProcessing(tabId);
            const hasResult = µb.UBE.hasResult(tabId);
            ...
            if ( isProcessing ) {
                ...
                const resultPred = µb.UBE.getResultPrediction(tabId).toLowerCase();
                let symbol = '✗';
                ...
                else if ( resultPred === 'unsafe' ) {
                    symbol = '⚠';
                    color = '#ff4444';
                }

                badge = badge ? badge + symbol : symbol;
            }
        }
