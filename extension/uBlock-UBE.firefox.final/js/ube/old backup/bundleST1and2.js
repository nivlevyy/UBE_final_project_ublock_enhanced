...
function isValidIPv4(ip) {
  return import_ip_address.Address4.isValid(ip);
}
function isValidIPv6(ip) {
  return import_ip_address.Address6.isValid(ip);
}
function isIPAddress(hostname) {
  return isValidIPv4(hostname) || isValidIPv6(hostname) ? 1 : 0;
}
function getCleanFullURL(parsedURL) {
  return parsedURL.hostname.replace(/^www\./, "") + parsedURL.pathname.replace(/\/+$/, "") + parsedURL.search.replace(/\/+$/, "");
}
...
function getCleanHostname(tldtsURL) {
  ...
  return cleanSubdomain ? `${cleanSubdomain}.${tldtsURL.domain}` : tldtsURL.domain;
}
function constructResultsDict(rawURL, cleanFullURL, tldtsURL, cleanHostname, parsedURL) {
  const parseResults = [
    lengthURL(cleanFullURL),
    numberOfSubdomains(tldtsURL.subdomain),
    lengthHostname(cleanHostname),
    isIPAddress(cleanHostname),
    isURLShortener(cleanHostname),
...
async function processURL(rawURL) {
  try {
    ...
    const cleanHostname = getCleanHostname(tldtsURL);
    const results = constructResultsDict(rawURL, cleanFullURL, tldtsURL, cleanHostname, parsedURL);
    const response = await fetch(
        "https://41xkaynei7.execute-api.eu-central-1.amazonaws.com/default/queryVT",
        {
          method: "POST",
          headers: {
    ...
    if (error instanceof TypeError) {
      ...
      throw new Error(`API or parsing error: ${error.message}`);
    }
  }
}
export {
