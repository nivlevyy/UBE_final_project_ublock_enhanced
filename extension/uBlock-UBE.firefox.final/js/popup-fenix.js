...
const updateHnSwitches = function() {
    ...
    dom.cl.toggle('#ubeToggle', 'on', popupData.ubeEnabled === true);
};
...
const toggleSections = function(more) {
    ...
    updateUBEStatus(); // re-open status bar if applicable
};
...
const toggleUBE = async function(ev) {
    const target = ev.currentTarget;
    const newState = !dom.cl.has(target, 'on');

    updateUBEToggleUI(target, newState);
...
const openUBEDebugPopup = function(ev) {
    ev.preventDefault();

    // uses popupDataFromTabId in messaging..
    messaging.send('popupPanel', {
        what: 'ubeOpenPopupWindow',
        tabId: popupData.tabId,
        hostname: popupData.pageHostname
    });
...
const updateUBEToggleUI = function(toggle, enabled) {
    if (!toggle || toggle.id !== 'ubeToggle') {
        return;
    }

    dom.cl.toggle(toggle, 'on', enabled);
    dom.attr(toggle, 'title', 'UBE Analysis ' + (enabled ? ' (Enabled)' : ' (Disabled)'));
};

const renderUBEStatus = function(status) {
    const statusElement = qs$('#ubeStatus');
    const statusIndicator = qs$('#ubeStatus .ube-status-indicator');
    const dotElement = qs$('#ubeStatus .ube-dot');
    const textElement = qs$('#ubeStatus .ube-text');
    const debugIcon = qs$('#ubeDebugIcon');
    const resultElement = qs$('#ubeResultText');

    const shouldShow = popupData.ubeEnabled || status.hasResult || status.processing;
    ...
    dom.cl.remove(dotElement, 'pending', 'processing', 'complete');
    dom.cl.remove(statusIndicator, 'has-results')
    dom.cl.remove(resultElement, 'safe', 'phishing', 'error');
    dom.text(resultElement, '');

    debugIcon.style.display = 'none';
    resultElement.style.display = 'none';

    if (status.processing) {
        dom.cl.add(dotElement, 'processing');
        dom.text(textElement, 'Analyzing page...');
    }
    else if (status.hasResult) {
        dom.cl.add(dotElement, 'complete');
        dom.text(textElement, 'Analysis complete');

        dom.cl.add(statusIndicator, 'has-results');
        debugIcon.style.display = 'inline-block';

        if (status.result && typeof status.result === 'string') {
            if (status.result.toLowerCase() === 'safe') {
                dom.cl.add(resultElement, 'safe');
                dom.text(resultElement, "✓ Safe");
            }
            else if (status.result.toLowerCase() === 'unsafe') {
                dom.cl.add(resultElement, 'unsafe');
                dom.text(resultElement, "⚠ Unsafe");
            }
            else {
                dom.cl.add(resultElement, 'error');
                dom.text(resultElement, "✗ N/A");
            }
        ...
        dom.cl.add(dotElement, 'pending');
        dom.text(textElement, 'Analysis pending');
    }
}

const updateUBEStatus = function() {
    browser.runtime.sendMessage(
        ...
        .then((status) => {
            renderUBEStatus(status);
    });
};

const initializeUBE = function() {
    const toggle = qs$('#ubeToggle');
    const isEnabled = popupData.ubeEnabled === true;

    updateUBEToggleUI(toggle, isEnabled);
};

if (browser.runtime) { // for real-time UBE status updating
    browser.runtime.onMessage.addListener((message) => {
        if (message?.source === 'ubeBackgroundScript' && message?.tabId === popupData.tabId) {
            if (message?.what === 'analysisStarted') {
                renderUBEStatus({
                    processing: true,
                    hasResult: false,
                });
            }
            else if (message?.what === 'analysisComplete') {
                renderUBEStatus({
                    processing: false,
                    hasResult: true,
...
dom.on('#ubeToggle', 'click', toggleUBE);
dom.on('#ubeDebugIcon', 'click', openUBEDebugPopup);
//dom.on('#ubeToggle', 'contextmenu', openUBEDebugPopup);
