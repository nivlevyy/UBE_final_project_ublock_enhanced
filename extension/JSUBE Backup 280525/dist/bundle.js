...
// UBE_Stage3.js
console.log("\u{1F525} Extension is running!");
document.body.style.border = "15px solid red";
var featuresAndLengthFull = {
  "favicon_check": 3,
  "url_anchor": 4,
  "links_in_tags": 10,
...
function detectSuspiciousJS(baseDomain) {
  const highRiskPatterns = [
    ...
    // Common in obfuscated payloads
  ];
  const mediumRiskPatterns = [
    /(?:window|document)\.location(\.href)?\s*=/i,
    // Unified location assignment
    /innerHTML\s*=/i,
    /outerHTML\s*=/i,
    /onbeforeunload/i,
    /window\.open\s*\(/i,
    // Opens phishing overlays
...
function extractRequestURLFeatures(baseDomain) {
  const resources = ["img", "source", "audio", "video", "embed", "iframe"].flatMap((tag) => Array.from(document.querySelectorAll(`${tag}[src]`)));
...
function getResults(type) {
  ...
  const features = type === INITIAL ? featuresAndLengthInitial : type === FULL ? featuresAndLengthFull : [];
  const results = {};
